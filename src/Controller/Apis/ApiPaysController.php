...
class ApiPaysController extends ApiInterface
{
    ...
    public function index(PaysRepository $villeRepository): Response
    {
        try {

            $pays = $villeRepository->findAll();
    ...
    public function create(Request $request, PaysRepository $paysRepository): Response
    {
        ...
        $userDTO = new PaysDTO();
        $userDTO->setLibelle($data['libelle']);
        $errorResponse = $this->errorResponse($userDTO);
        if ($errorResponse !== null) {
            return $errorResponse; // Retourne la réponse d'erreur si des erreurs sont présentes
        }else{
            $paysRepository->save($userDTO, true);
        }

        return $this->responseData($userDTO, 'group1', ['Content-Type' => 'application/json']);
    }


    #[Route('/update/{id}', methods: ['PUT', 'POST'])]
    /**
     * Permet de mettre à jour un(e) pays.
     */
    #[OA\Response(
        response: 200,
        description: 'Returns the rewards of an user',
    ...
    #[OA\Tag(name: 'pays')]
    //#[Security(name: 'Bearer')]
    public function update(Request $request, Pays $pays, PaysRepository $villeRepository): Response
    {
        try {
